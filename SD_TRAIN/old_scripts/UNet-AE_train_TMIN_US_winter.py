...
for sea in seasons:
    ...
    # hist dict
    hist_total = {'loss': [], 'HR_temp_loss': [], 'HR_elev_loss': [], 'HR_temp_mean_absolute_error': [], 'HR_elev_mean_absolute_error': [], 
                  'val_loss': [], 'val_HR_temp_loss': [], 'val_HR_elev_loss': [], 'val_HR_temp_mean_absolute_error': [], 'val_HR_elev_mean_absolute_error': []}

    # first round trining
    ## model definition
    save_dir = '/glade/work/ksha/data/Keras/BACKUP/'
    model_name = 'UNET_{}_B4_{}'.format(VAR, sea)
    model = au.UNET_AE(layer_N, c_size, dropout=True, rate=0.1)
    #model = keras.models.load_model(save_dir+model_name+'.hdf') # restart option
    ## opt
    opt_adam = keras.optimizers.Adam(lr=lr[0])
    ## callback
    callbacks = [keras.callbacks.EarlyStopping(monitor='val_HR_temp_loss', min_delta=0.0000001, patience=10, verbose=True),
    ...
    model.compile(loss=[keras.losses.mean_absolute_error, keras.losses.mean_absolute_error], loss_weights=[1, 1], optimizer=opt_adam, metrics=[keras.losses.mean_absolute_error])
    ...
    for i in range(15):
        ...
        temp_hist1 = model.fit_generator(generator=gen_train, validation_data=gen_valid, callbacks=callbacks, 
                                         ...
                                         verbose=1, shuffle=True, max_queue_size=18, workers=18)
