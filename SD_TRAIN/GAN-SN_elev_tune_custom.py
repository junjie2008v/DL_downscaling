...
def XIN(data, grid_shape=(200, 96, 96, 3)):
...
min_del = 0
max_tol = 2 # early stopping with patience
...
# overwrite flags
input_flag = [True, True] # LR T2, HR elev, LR elev
output_flag = [True, False] # HR T2
...
for sea in seasons:
    ...
    W = tu.dummy_loader(temp_dir+'STRANS_G_TMEAN_{}.hdf'.format(sea))
    ...
    # Filepath
    file_path = BATCH_dir
    trainfiles = glob(file_path+'ETOPO_BATCH_96*.npy')
    validfiles = glob(file_path+'ETOPO_BATCH_96*.npy')
    #
