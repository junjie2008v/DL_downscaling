...
if N_output > 1:
    raise ValueError('XNet accepts only one target')
    
# UNet model macros
N = [38, 68, 98, 128] # hidden layer numbers (symetrical)
...
for sea in seasons:
    ...
    # model name
    model_name = 'XNET{}_{}_{}'.format(N_input, VAR, sea)
    model_path = temp_dir+model_name+'.hdf' # model checkpoint
    train_path = temp_dir+model_name+'.npy' # train history
    # full list of files
    trainfiles = glob(file_path+'{}_BATCH_*_TORI*_{}*.npy'.format(VAR, sea)) # e.g., PCT_BATCH_128_VORIAUG_mam30.npy
    ...
    model = mu.XNET(N, (None, None, N_input)) # model = keras.models.load_model(save_dir+model_name+'.hdf') # restart options
    # optimizer
