...
for sea in seasons:
    ...
    # tuned model
    model_name_tune = 'UNET{}_{}_{}_clean'.format(N_input, VAR, sea)
    ...
    # callbacks, tol=5
    callbacks = [keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=2, verbose=True),
    ...
    trainfiles = glob(file_path+'TMEAN_BATCH_*_TORI-ERA-clean_{}*'.format(sea))
    # shuffle filenames
    shuffle(trainfiles)
    shuffle(validfiles)
    # generators
