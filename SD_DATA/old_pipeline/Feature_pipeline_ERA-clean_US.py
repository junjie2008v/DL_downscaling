...
vars  = ['TMEAN']
gaps  = [16, 16] # mean grid point distance between croppings (base number, will be randomly moved around)
sizes = [64, 96]

# clean-up
if del_old:
    cmd = 'rm {}*ERA-clean*npy'.format(BATCH_dir); print(cmd)
    subprocess.call(cmd, shell=True)
    cmd = 'rm {}*ERA-clean*npy'.format(BATCH_dir+'temp_batches/'); print(cmd)
    subprocess.call(cmd, shell=True)

# train date range
# datetime info
N_days = 365 + 366 + 365 + 365 + 243 # 2015-2020 (period ending)
...
for var in vars:
    ...
    # T2 fields
    input_train_3d = {}
    input_valid_3d = {}#PRISM_TMEAN_clean_2015_2020.hdf
    keys_3d = ['{}_4km'.format(var), '{}_REGRID'.format(var)]
    with h5py.File(ERA_dir+'ERA_US_{}_clean_2015_2020.hdf'.format(var), 'r') as hdf_io:
        for key in keys_3d:
    ...
    for i, size in enumerate(sizes):
        ...
        for sea in seasons:
            ...
            for domain in domains:
                ...
                NAME_train = '{}_BATCH_{}_T{}-ERA-clean_{}'.format(var, size, domain, sea)
                NAME_valid = '{}_BATCH_{}_V{}-ERA-clean_{}'.format(var, size, domain, sea)
                ...
                FEATURE_train = pu.random_cropping(input_train_3d, keys_3d, input_2d, keys_2d, mask[domain], 
                                                   size, gap, ind_train, rnd_range=4, clim=False)
                ...
                FEATURE_valid = pu.random_cropping(input_valid_3d, keys_3d, input_2d, keys_2d, mask[domain], 
                                                   size, gap, ind_valid, rnd_range=4, clim=False)
