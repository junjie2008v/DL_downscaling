...
def CLOSS(IN):
    def loss(data, y_pred):
        ...
        lr_min = tf.reduce_min(y_lr)
        lr_max = tf.reduce_max(y_lr)

        norm_lr = (y_lr-lr_min)/(lr_max-lr_min)
        norm_pred = (y_pred-pred_min)/(pred_max-pred_min)

        return 0.1*tf.image.ssim(norm_lr, norm_pred, max_val=1.0)+K.mean(K.square(y_true - y_pred))
    return loss

def dummy_g_loader(N_input, VAR, sea):
    # model name
...
for sea in seasons:
    ...
    GAN.compile(loss=[CLOSS(GAN_IN), keras.losses.binary_crossentropy], 
    ...
    ssim_flag = [True, True, False, False, False, False]
    ...
    # model names
    G_name = '{}_G_{}_{}'.format(key, VAR, sea)
    D_name = '{}_D_{}_{}'.format(key, VAR, sea)
    G_path = temp_dir+G_name+'.hdf'
    D_path = temp_dir+D_name+'.hdf'
    hist_path = temp_dir+'{}_LOSS_{}_{}.npy'.format(key, VAR, sea)
    ...
    for i in range(epochs):
        ...
        if i == 0:
            ...
            record = 10*record # x10 for not early stopped at the begining
            print('Initial validation loss x10 = {}'.format(record)) 
        ...
        for j, name in enumerate(trainfiles):
            ...
            gan_target = [X[..., ssim_flag], dummy_good]
