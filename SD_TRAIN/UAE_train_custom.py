'''
parser arguments: variable, seasons, 
'''
...
# UNet model macros
N = [56, 112, 224, 448] # hidden layer numbers (symetrical)
...
for sea in seasons:
    ...
    hist_total = {'loss': [], 'HR_temp_loss': [], 'HR_elev_loss': [], 'HR_temp_mean_absolute_error': [], 'HR_elev_mean_absolute_error': [], 
                  'val_loss': [], 'val_HR_temp_loss': [], 'val_HR_elev_loss': [], 
                  'val_HR_temp_mean_absolute_error': [], 'val_HR_elev_mean_absolute_error': []}
    
    # validation set generator
    gen_valid = tu.grid_grid_gen_multi(validfiles[:num_valid], labels, input_flag, output_flag)

    # model initialization
    model = mu.UNET_AE(N, (None, None, N_input), output_channel_num=N_output)
    # optimizer
    ...
    # compile
    # MAE is pixel number independent, but in case can use: loss_weights=[1, 1/N_output]
    ...
    for i in range(10):
        ...
        gen_train = tu.grid_grid_gen_multi(trainfiles[:num_train], labels, input_flag, output_flag)
